...
public class MuxWar extends Event implements Listener {
    ...
    public MuxWar(final MuxEvents e) {
        super(e);
        name = "MuxWar";
        ...
        breakBlocks = true;
        pvp = true;
        sb = ms.getScoreboard().createScoreboard("§7§lMux§d§lWar");
        sb.setLook((p, board) -> {
            sb.setLine(board, "");
    ...
    private void checkScore() {
        ...
        if (redpoints >= maxscore) {
            ms.broadcastMessage("§d§lMuxWar>§7 Team §cROT §7hat §agewonnen§7!", null, MuxSystem.Priority.NORMAL);
            winner = MuxEvents.Team.RED;
            win();
        } else if (bluepoints >= maxscore) {
            ms.broadcastMessage("§d§lMuxWar>§7 Team §9BLAU §7hat §agewonnen§7!", null, MuxSystem.Priority.NORMAL);
    ...
    private void lostFlag(final Player p) {
        if (blueflagplayer == p) {
            ...
            sendEventMessage("§d§lMuxWar>§7 Die §9BLAUE FLAGGE §7wurde zurückerobert.");
        } else if (redflagplayer == p) {
            redflagplayer = null;
            setFlag(warflagred, DyeColor.RED);
            sendEventMessage("§d§lMuxWar>§7 Die §cROTE FLAGGE §7wurde zurückerobert.");
        }
    }
    ...
    private void win() {
        for (final Map.Entry<String, MuxEvents.Team> entry : teams.entrySet()) {
            ...
            resetPlayer(p);
            if (t == winner) {
                ms.sendTitle(p, "§d§lMuxWar", 10, 120, 10);
    ...
    @Override
    public void start() {
        final String map = r.nextBoolean() ? "" : "2";
        final boolean open = (warspawnblue = ms.getLoc("warspawnblue" + map)) != null && (warspawnred = ms.getLoc("warspawnred" + map)) != null
                && (warflagblue = ms.getLoc("warflagblue" + map)) != null && (warflagred = ms.getLoc("warflagred" + map)) != null;
        if (open == false) {
            stopEvent(true);
            return;
        }
    ...
    public boolean join(final Player p) {
        ...
        pi.addItem(new ItemStack(Material.ARROW, 30));
        p.updateInventory();
    ...
    public void onDamage(final EntityDamageEvent e) {
        ...
        if (e instanceof EntityDamageByEntityEvent) {
            ...
            } else if (damageByEntity.getDamager() instanceof Projectile) {
                final Projectile arrow = (Projectile) damageByEntity.getDamager();
        ...
        if (p.getHealth() - e.getFinalDamage() <= 0D) {
            e.setCancelled(true);
            if (damager != null) {
                ms.showItemBar(damager, "§fDu hast " + (t == MuxEvents.Team.BLUE ? "§9" : "§c") + p.getName() + " §fbesiegt. " + (t == MuxEvents.Team.BLUE ? "§c" : "§9") + ("(+1)"));
    ...
    public void onInteract(final PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.LEFT_CLICK_BLOCK || e.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (p == blueflagplayer && b.getLocation().equals(warflagred)) {
                ...
                sendEventMessage("§d§lMuxWar>§7 Team §cROT §7hat die §9BLAUE FLAGGE §7erobert! §c(+10)");
                redpoints += 10;
                ...
                sendEventMessage("§d§lMuxWar>§7 Team §9BLAU §7hat die §cROTE FLAGGE §7erobert! §9(+10)");
                bluepoints += 10;
            } else {
                return;
            }
    ...
    public void onBreak(final BlockBreakEvent e) {
        ...
        if (b.getLocation().equals(warflagblue)) {
            ...
            sendEventMessage("§d§lMuxWar>§7 Der Spieler §c" + p.getName() + " §7hat die §9BLAUE FLAGGE §7geklaut!");
            ...
            sendEventMessage("§d§lMuxWar>§7 Der Spieler §9" + p.getName() + " §7hat die §cROTE FLAGGE §7geklaut!");
            ms.sendTitle(p, "§f", 10, 80, 10);
